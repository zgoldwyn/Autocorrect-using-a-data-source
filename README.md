...
## Design Decisions
I decided to use a Trie over a list because it has O(k) time complexity for search, given k is the already typed string. This differs from using a list of possible words and iterating through because the time complexity would be O(n) for any case, even if there are only a few valid word options. 
